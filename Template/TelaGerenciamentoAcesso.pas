...
type
  ...
  TFormTelaGerenciamentoAcesso = class(TForm)
    ...
    procedure cmbTelasChange(Sender: TObject);
  private
    OP: TOperacao;
    UsuListIndex: Integer;
    CodigosExistentes: string;
    ListaTelas: TStringList;
    procedure MontaTelasPermissao;
...
begin
  ...
  begin
    ...
    SQLGeral.Close;
    SQLGeral.SQL.Text := 'select * from USUARIOPERMISSOES where USUAICOD = ' + CodUsuario + ' and USPEA60NOMETELA = ''' + ListaTelas[cmbTelas.ItemIndex]+ '''';
    SQLGeral.Open;
    ...
    begin
      ...
      SQLAcessoUSPEA60NOMETELA.AsString := ListaTelas[cmbTelas.ItemIndex];
...
procedure TFormTelaGerenciamentoAcesso.MontaTelasPermissao;
begin
  ListaTelas := TStringList.Create;
  cmbTelas.Clear;

  if ExecSql(' select 1 from USUARIOPERMISSOES WHERE uspea60nometela = ' + QuotedStr(DM.JanelaAtiva.Nome)).IsEmpty then
  begin
    ListaTelas.Add(DM.JanelaAtiva.Nome);
    cmbTelas.Items.Add(lblJanelaAtiva.Caption);
  end
  else
    ExecSql(' UPDATE USUARIOPERMISSOES SET FANTASIA = ' + QuotedStr(lblJanelaAtiva.Caption)
      + ' WHERE uspea60nometela = ' + QuotedStr(DM.JanelaAtiva.Nome), 1);

  with ExecSql(' select distinct COALESCE(FANTASIA,uspea60nometela) as Descricao, uspea60nometela from USUARIOPERMISSOES ') do
  begin
    while not eof do
    begin
      ListaTelas.Add(FieldByName('uspea60nometela').AsString);
      cmbTelas.Items.Add(FieldByName('Descricao').AsString);
      next;
    end;
  end;

  cmbTelas.ItemIndex := ListaTelas.IndexOf(DM.JanelaAtiva.Nome);
end;

procedure TFormTelaGerenciamentoAcesso.AbrirDados;
begin
  lstUsuario.Clear;
  lstUsuarioSel.Clear;
  CodigosExistentes := '';
  ...
  SQLGeral.SQL.Text := 'select CLASSE.CLASSA60DESCRICAO,CLASSE.CLASSICOD,USUARIO.USUAICOD,USUARIO.USUAA60LOGIN from USUARIO ' +
    ...
    'where USUARIO.USUAICOD in (select USUAICOD from USUARIOPERMISSOES where USPEA60NOMETELA = ''' + ListaTelas[cmbTelas.ItemIndex] + ''') ' +
  ...
  begin
    lstUsuarioSel.Items.Add(SQLGeral.FieldByName('CLASSA60DESCRICAO').AsString + ' > ' + SQLGeral.FieldByName('USUAICOD').AsString
       + '-' + SQLGeral.FieldByName('USUAA60LOGIN').AsString);
...
procedure TFormTelaGerenciamentoAcesso.cmbTelasChange(Sender: TObject);
begin
  ...
  lblJanelaAtiva.Caption := cmbTelas.Text;
end;
