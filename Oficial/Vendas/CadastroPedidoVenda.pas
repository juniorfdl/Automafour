...
uses
  ...
  jpeg, DBCtrls, EDBNum, RxDBComb, ToolEdit, RXDBCtrl, VarSys, FormResources,
  CurrEdit, RxLookup, UCrpe32, ValEdit, Placemnt, ppDB, ppDBPipe, ppDBBDE,
  ppStrtch, ppMemo, ppBands, ppCtrls, ppPrnabl, ppClass, ppCache, ppComm,
  ppViewr, ppRelatv, ppProd, ppReport, ppSubRpt, RDprint, Serial,
...
type
  TFormCadastroPedidoVenda = class(TFormCadastroTEMPLATE)
    ...
    procedure EditCdProdutoKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
    ...
    procedure DBEditNomeClienteKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
    procedure CorrigeNomesdeClientesVazios1Click(Sender: TObject);
    procedure DSSQLPedidoVendaItemStateChange(Sender: TObject);
    procedure PedidoTotais1Click(Sender: TObject);
  private
...
uses
  DataModulo, CadastroCliente, CadastroVendedor, CadastroTransportadora,
...
begin
  ...
  if not SQLVendedor.Active then
    SQLVendedor.Open;
  if not SQLCliente.Active then
    SQLCliente.Open;
  if not SQLTransportadora.Active then
    SQLTransportadora.Open;
  if not SQLVeiculo.Active then
    SQLVeiculo.Open;
  if not SQLSerie.Active then
    SQLSerie.Open;
  if not SQLRota.Active then
    SQLRota.Open;
...
procedure TFormCadastroPedidoVenda.DBEdit6KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
  if Key = VK_F2 then
    BtnCliente.Click;
...
procedure TFormCadastroPedidoVenda.SQLTemplateBeforeEdit(DataSet: TDataSet);
begin
...
procedure TFormCadastroPedidoVenda.SQLTemplateBeforeDelete(DataSet: TDataSet);
begin
...
procedure TFormCadastroPedidoVenda.MnCancelarPedidodeVendaClick(Sender: TObject);
begin
  inherited;
...
procedure TFormCadastroPedidoVenda.EtiquetasdeCdigodeBarras1Click(Sender: TObject);
begin
  inherited;
...
procedure TFormCadastroPedidoVenda.SQLTemplatePDVDN2VLRDESCPROMChange(Sender: TField);
begin
  inherited;
...
procedure TFormCadastroPedidoVenda.SQLContasReceberBeforePost(DataSet: TDataSet);
begin
  inherited;
...
procedure TFormCadastroPedidoVenda.ImprimirPedidodeVenda1Click(Sender: TObject);
begin
  inherited;
...
procedure TFormCadastroPedidoVenda.DSTemplateDataChange(Sender: TObject; Field: TField);
begin
  inherited;
...
begin
  if not SQLOrcamento.Active then
    SQLOrcamento.Open;
{  SQLOrcamento.Close ;
  ...
  DM.TblPedidoCabPlano.asString := SQLTemplatePLRCICOD.AsString + ' - ' + SQLTemplatePlanoRecebimentoLookUp.asString;
  ...
  DM.TblPedidoCabVendedor.asString := SQLOrcamento.FieldByName('VENDICOD').AsString + ' - ' + SQLLocate('VENDEDOR', 'VENDICOD', 'VENDA60NOME', SQLOrcamento.FieldByName('VENDICOD').AsString);
  DM.TblPedidoCabRota.asString := SQLOrcamento.FieldByName('ROTAICOD').AsString + ' - ' + SQLLocate('ROTA', 'ROTAICOD', 'ROTAA60NOME', SQLOrcamento.FieldByName('ROTAICOD').AsString);
  ...
  if not SQLOrcamentoItem.Active then
  ...
  begin
    ...
    DM.TblPedidoItensVlrDesc.Value     := SQLOrcamentoItem.FieldbyName('PVITN2VLRDESC').Value;
    DM.TblPedidoItensVlrTotal.Value := (SQLOrcamentoItem.FieldbyName('PVITN2VLRUNIT').Value) * SQLOrcamentoItem.FieldbyName('PVITN3QUANT').Value;

    DM.TblPedidoItensUn.Value := SQLLocate('UNIDADE', 'UNIDICOD', 'UNIDA5DESCR', SQLLocate('PRODUTO', 'PRODICOD', 'UNIDICOD', SQLOrcamentoItem.FieldByName('PRODICOD').AsString));
  ...
  begin
    if not SQLParcelasPrazo.Active then
...
procedure TFormCadastroPedidoVenda.SQLContasReceberPostError(DataSet: TDataSet; E: EDatabaseError; var Action: TDataAction);
var
  ID: string;
begin
  inherited;
...
procedure TFormCadastroPedidoVenda.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
  DataSetLookup := Nil;
end;
...
procedure TFormCadastroPedidoVenda.MnPedidoOramentoemSequenciaClick(Sender: TObject);
var
  Linha, vCol: integer;
begin
  inherited;
...
procedure TFormCadastroPedidoVenda.SQLPedidoVendaItemNewRecord(DataSet: TDataSet);
begin
  SQLPedidoVendaItemPDVDA13ID.Value := SQLTemplatePDVDA13ID.Value;
  if SQLPedidoItens.Active then
    SQLPedidoItens.Close;
...
procedure TFormCadastroPedidoVenda.SQLTemplateBeforePost(DataSet: TDataSet);
begin
...
procedure TFormCadastroPedidoVenda.MnTrrocarVendedoreRotaPadraoClick(Sender: TObject);
begin
  inherited;
...
procedure TFormCadastroPedidoVenda.MnTrocarStatusdoPedidoparaFaturadoClick(Sender: TObject);
begin
  inherited;
...
procedure TFormCadastroPedidoVenda.BtnExcluirParcAtuaisClick(Sender: TObject);
begin
  inherited;
...
procedure TFormCadastroPedidoVenda.SQLPedidoVendaItemPRODICODChange(Sender: TField);
var
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          SQLGeral.Close;
          SQLGeral.sql.Clear;
          SQLGeral.SQL.Add('Select * from PedidoVendaItem where PDVDA13ID = ''' + FormTelaPedidoParcial.TblQtdePDVDA13ID.Value + ''' and PRODICOD = ' + FormTelaPedidoParcial.TblQtdePRODICOD.Asstring);
          SQLGeral.Open;
          SQLGeral.Edit;
          SQLGeral.FindField('PVITN3QUANTVEND').Value := sqlgeral.FindField('PVITN3QUANTVEND').Value - FormTelaPedidoParcial.TblQtdeQTDEATENDIDO.Value;
          SQLGeral.Post;
        end;
...
procedure TFormCadastroPedidoVenda.SQLContasReceberNewRecord(DataSet: TDataSet);
begin
  inherited;
...
procedure TFormCadastroPedidoVenda.MnIgualarSaldoaEntregarcomoSaldoPedidoClick(Sender: TObject);
begin
  inherited;
  if SQLTemplatePDVDCSTATUS.Value = 'P' then
  begin
...
procedure TFormCadastroPedidoVenda.MnVisualizarPedidoOrcamentoAtualClick(Sender: TObject);
begin
  inherited;
  if (DM.SQLTerminalAtivo.FieldByName('TERMCTIPO').AsString <> '') then
  begin
...
procedure TFormCadastroPedidoVenda.SQLPedidoVendaItemPVITN2VLRUNITChange(Sender: TField);
begin
  inherited;
  if (SQLPedidoVendaItemPVITN2VLRUNIT.Value > 0) and (SQLPedidoVendaItemPVITN3QUANT.Value > 0) and (SQLPedidoVendaItem.State in dsEditModes) then
...
begin
  SQLTotal.Close;
  SQLTotal.SQL.Clear;
  SQLTotal.SQL.Add('Select Sum(PVITN3TOTVENDITEM - coalesce(PVITN2VLRDESC,0)) as TotalGeral,Sum(PVITN2VLRDESC) as TotalDesconto ,  Sum(PRODN3PESOBRUTO*(PVITN3QUANT+PVITN3QTDBONIF)) as PesoTotal from PedidoVendaItem where PDVDA13ID = ''' + SQLTemplatePDVDA13ID.Value + '''');
  SQLTotal.Prepare;
  SQLTotal.Open;
  try
    SQLtemplate.Edit;
    ...
    try
      SQLTemplatePDVDN2VLRDESC.value := SQLTotal.FieldByName('TotalDesconto').Value;
    except
      SQLTemplatePDVDN2VLRDESC.value := 0;
    end;
...
procedure TFormCadastroPedidoVenda.SQLTemplateBeforeOpen(DataSet: TDataSet);
begin
  inherited;
  if RdAbertos.Checked then
    SQLtemplate.MacroByName('MSituacao').Value := '(PDVDCSTATUS = ''A'' or PDVDCSTATUS = ''E'' or PDVDCSTATUS = ''P'')';
  if RdFaturado.Checked then
...
procedure TFormCadastroPedidoVenda.SQLTemplateCLIEA13IDChange(Sender: TField);
begin
  inherited;
  try
...
procedure TFormCadastroPedidoVenda.SQLPedidoVendaItemUNIDICODChange(Sender: TField);
begin
  inherited;
  ...
  begin
    ...
    +' PRODN3PESOBRUTO, PRODA60CODBAR from produto where prodicod = ' + SQLPedidoVendaItemPRODICOD.AsString;
    dm.zConsulta.Open;
...
procedure TFormCadastroPedidoVenda.SQLPedidoVendaItemCalcFields(DataSet: TDataSet);
begin
  inherited;
...
procedure TFormCadastroPedidoVenda.EditCdProdutoKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
  if Key = VK_F2 then
    AcessaProduto.Click;
...
procedure TFormCadastroPedidoVenda.SQLContasReceberCalcFields(DataSet: TDataSet);
begin
  inherited;
...
var
  xPesoTotalMix, xPesoTotalPedido, xDesconto: double;
begin
  inherited;
  xPesoTotalMix := 0;
  xPesoTotalPedido := 0;
...
procedure TFormCadastroPedidoVenda.BTNRecalcularPedidoClick(Sender: TObject);
var
  xUnidade1, xUnidade2: string;
begin
  inherited;
...
procedure TFormCadastroPedidoVenda.DBEditNomeClienteKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
  if Key = VK_F2 then
    BtnCliente.Click;
...
procedure TFormCadastroPedidoVenda.CorrigeNomesdeClientesVazios1Click(Sender: TObject);
begin
  inherited;
  while not SQLTemplate.eof do
  begin
...
procedure TFormCadastroPedidoVenda.DSSQLPedidoVendaItemStateChange(Sender: TObject);
begin
  inherited;
  HabilitaItens(((SQLTemplate.State in dsEditModes) or (SQLPedidoVendaItem.State in dsEditModes)));
end;

procedure TFormCadastroPedidoVenda.PedidoTotais1Click(Sender: TObject);
begin
  inherited;
  SQLPedidoItens.Close;
  SQLPedidoItens.SQL.Text := 'Select * From PEDIDOVENDAITEM Where PDVDA13ID = ''' + SQLTemplate.FieldByName('PDVDA13ID').AsString + '''';
  ...
  if SQLTemplatePDVDCTIPO.asString = 'P' then
    ...
    DM.TblPedidoCabPEDIDOORCAMENTO.AsString := 'ORÇAMENTO NRO:';
  DM.TblPedidoCabPedICod.AsString := SQLTemplatePDVDA13ID.AsString;
  DM.TblPedidoCabEmitente_Nome.asString := DM.SQLEmpresaEMPRA60NOMEFANT.AsString;
  DM.TblPedidoCabEmitente_Ender.asString := DM.SQLEmpresaEMPRA60END.asString;
  DM.TblPedidoCabEmitente_Bairro.asString := DM.SQLEmpresaEMPRA60BAI.asString;
  DM.TblPedidoCabEmitente_Cidade.asString := DM.SQLEmpresaEMPRA60CID.asString;
  ...
  DM.TblPedidoCabEmitente_IE.asString := DM.SQLEmpresaEMPRA20IE.asString;
  DM.TblPedidoCabEmitente_Fone.asString := Dm.SQLEmpresaEMPRA20FONE.asString;
  DM.TblPedidoCabDtEmissao.Value := SQLTemplatePDVDDEMISSAO.Value;
  if DM.ProcuraRegistro('CLIENTE', ['CLIEA13ID'], [SQLTemplateCLIEA13ID.Value], 1) then
  begin
    DM.TblPedidoCabDestinatario_Nome.asString := DM.SQLTemplate.FieldByName('CLIEA60RAZAOSOC').AsString;
    DM.TblPedidoCabDestinatario_Endereco.asString := DM.SQLTemplate.FieldByName('CLIEA60ENDRES').AsString;
    DM.TblPedidoCabDestinatario_Bairro.asString := DM.SQLTemplate.FieldByName('CLIEA60BAIRES').AsString;
  ...
  if SQLTemplatePDVDCTIPOFRETE.asString = 'C' then
    ...
    DM.TblPedidoCabTipoFrete.asString := '2';
  DM.TblPedidoCabValorFrete.Value := SQLTemplatePDVDN2VLRFRETE.AsFloat;
  DM.TblPedidoCabTotalDesconto.Value := SQLTemplatePDVDN2VLRDESC.AsFloat + SQLTemplatePDVDN2VLRDESCPROM.AsFloat;
  DM.TblPedidoCabTotal.Value := SQLTemplatePDVDN2TOTPROD.AsFloat;
  DM.TblPedidoCabObservacoes.asString := SQLTemplatePDVDTOBS.asString;
  DM.TblPedidoCabVendedor.asString := SQLTemplateVENDICOD.AsString + ' - ' + SQLLocate('VENDEDOR', 'VENDICOD', 'VENDA60NOME', SQLTemplateVENDICOD.AsString);
  DM.TblPedidoCabRota.asString := SQLTemplateROTAICOD.AsString + ' - ' + SQLLocate('ROTA', 'ROTAICOD', 'ROTAA60NOME', SQLTemplateROTAICOD.AsString);
  //  DM.TblPedidoCabEmail.asString                  := AnsiLowerCase(SQLTemplateVendedorEmailLookUp.asString);
  DM.TblPedidoCabEmitente_Email.AsString := LowerCase(DM.SQLEmpresaEMPRA60EMAIL.AsString);
  DM.TblPedidoCabEmitente_URL.AsString := LowerCase(DM.SQLEmpresaEMPRA60URL.AsString);
  DM.TblPedidoCabEmitenteLogo.AsVariant := DM.SQLEmpresaEMPRBLOGOTIPO.AsVariant;
  ...
  begin
    DM.TblPedidoItens.Append;
    DM.TblPedidoItensVlrUnitario.Value := SQLPedidoVendaItemPVITN2VLRUNIT.Value;
    DM.TblPedidoItensVlrDesc.Value     := SQLPedidoVendaItemPVITN2VLRDESC.Value;
    DM.TblPedidoItensVlrTotal.Value := (SQLPedidoVendaItemPVITN2VLRUNIT.Value) * SQLPedidoVendaItemPVITN3QUANT.Value;

    DM.TblPedidoItensUn.Value := SQLLocate('UNIDADE', 'UNIDICOD', 'UNIDA5DESCR', SQLLocate('PRODUTO', 'PRODICOD', 'UNIDICOD', SQLPedidoVendaItemPRODICOD.AsString));

    DM.TblPedidoItensQtd1.Value := SQLPedidoVendaItemPVITN3QUANT.Value;
    DM.TblPedidoItensPedICod.Value := SQLPedidoVendaItemPDVDA13ID.Value;
    DM.TblPedidoItensDescricao.Value := SQLLocate('PRODUTO', 'PRODICOD', 'PRODA60DESCR', SQLPedidoVendaItemPRODICOD.AsString);
    DM.TblPedidoItensReferencia.Value := SQLLocate('PRODUTO', 'PRODICOD', 'PRODA60REFER', SQLPedidoVendaItemPRODICOD.AsString);
    DM.TblPedidoItensCodigo.Value := SQLPedidoVendaItemPRODICOD.AsInteger;
    DM.TblPedidoItensCodItem.Value := SQLPedidoVendaItemPVITIITEM.AsInteger;
    DM.TblPedidoItens.Post;
    SQLPedidoVendaItem.Next;
  end;
...
//  Report.DiscardSavedData := True;
  ...
  Report.ReportTitle := 'Pedido';
  Report.WindowStyle.Title := 'Pedido';
//  if Dm.SQLTerminalAtivo.FieldByName('TERMA60IMPPEDIDO').AsString <> '' then
//  begin
//    DM.Report.Output := toPrinter;
//    DM.Report.Printer.Name := Dm.SQLTerminalAtivo.FieldByName('TERMA60IMPPEDIDO').AsString;
