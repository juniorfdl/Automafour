...
begin
  ...
  if SQLGeraMail.IsEmpty then
    begin
      Application.MessageBox('Não existe nenhum Email a ser enviado !','Informação',MB_OK + MB_ICONINFORMATION + MB_SYSTEMMODAL);
      Abort;
    end;

  if SQLConfigEmail.FieldByName('CFEMTMAILREMET').AsString = '' then
    begin
      Application.MessageBox('Informe o Email do Rementente nas configurações...','Informação',MB_OK + MB_ICONINFORMATION + MB_SYSTEMMODAL);
      Abort;
    end;

  if SQLConfigEmail.FieldByName('CFEMA60REMETENTE').AsString = '' then
    begin
      Application.MessageBox('Informe o Nome do Rementente nas configurações...','Informação',MB_OK + MB_ICONINFORMATION + MB_SYSTEMMODAL);
      Abort;
    end;

  if SQLConfigEmail.FieldByName('CFEMIPORTA').AsString = '' then
    begin
      Application.MessageBox('Informe a Porta de comunicação nas configurações...','Informação',MB_OK + MB_ICONINFORMATION + MB_SYSTEMMODAL);
      Abort;
    end;

  if SQLConfigEmail.FieldByName('CFEMTUSUARIO').AsString = '' then
    begin
      Application.MessageBox('Informe o nome do Usuário nas configurações...','Informação',MB_OK + MB_ICONINFORMATION + MB_SYSTEMMODAL);
      Abort;
    end;

  if SQLConfigEmail.FieldByName('CFEMTHOST').AsString = '' then
    begin
      Application.MessageBox('Informe o Servidor de Email nas configurações...','Informação',MB_OK + MB_ICONINFORMATION + MB_SYSTEMMODAL);
      Abort;
    end;
  ...
  if SQLGeraMail.FieldByName('MAILTPATHANEXO').AsString <> '' then
    TIdAttachment.Create(IdMsgSend.MessageParts, SQLGeraMail.FieldByName('MAILTPATHANEXO').AsString);
...
begin
  ...
  LabelEnviar.Caption := IntToStr(0) + ' Mensagen(s) à enviar ';
  LabelEnviar.Update;
...
begin
  inherited;
  Application.MessageBox('Conexão falhou !','Informa',MB_OK + MB_ICONINFORMATION + MB_SYSTEMMODAL);
  Abort;
end;
...
begin
  inherited;
  Application.MessageBox('O sistema não pode enivar o email !','Informa',MB_OK + MB_ICONINFORMATION + MB_SYSTEMMODAL);
  Abort;
end;
...
begin
  inherited;
  Application.MessageBox('Autenticação Falhou, verifique as configurações !','Informa',MB_OK + MB_ICONINFORMATION + MB_SYSTEMMODAL);
  Abort;
end;
