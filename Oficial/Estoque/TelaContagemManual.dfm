object FormTelaContagemManual: TFormTelaContagemManual
  Left = 160
  Top = 0
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  Caption = 'Entrada Manual de Dados no Invent'#225'rio'
  ClientHeight = 672
  ClientWidth = 1099
  Color = 16249066
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  Visible = True
  OnClose = FormClose
  OnCreate = FormCreate
  OnKeyPress = FormKeyPress
  PixelsPerInch = 96
  TextHeight = 13
  object SpeedButton2: TSpeedButton
    Tag = 3
    Left = 681
    Top = 3
    Width = 74
    Height = 25
    Cursor = crHandPoint
    Hint = 'Fechar Tela'
    Caption = '&Fechar'
    Flat = True
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    Glyph.Data = {
      
      0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C}
    ParentFont = False
    OnClick = BtnFecharTelaClick
  end
  object GroupBox1: TGroupBox
    Left = 0
    Top = 210
    Width = 1099
    Height = 134
    Align = alTop
    Caption = 'Produto'
    Color = 16249066
    Font.Charset = DEFAULT_CHARSET
    Font.Color = 8404992
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    TabOrder = 0
    object Label2: TLabel
      Left = 11
      Top = 17
      Width = 67
      Height = 13
      Caption = 'C'#243'd Interno'
      FocusControl = DBEdit1
    end
    object Label3: TLabel
      Left = 131
      Top = 17
      Width = 63
      Height = 13
      Caption = 'C'#243'd Antigo'
      FocusControl = DBEdit2
    end
    object Label4: TLabel
      Left = 255
      Top = 17
      Width = 63
      Height = 13
      Caption = 'C'#243'd Barras'
      FocusControl = DBEdit3
    end
    object Label5: TLabel
      Left = 11
      Top = 54
      Width = 58
      Height = 13
      Caption = 'Descri'#231#227'o'
      FocusControl = DBEdit4
    end
    object Label10: TLabel
      Left = 11
      Top = 93
      Width = 35
      Height = 13
      Caption = 'Grupo'
      FocusControl = DBEdit5
    end
    object Label11: TLabel
      Left = 217
      Top = 93
      Width = 55
      Height = 13
      Caption = 'Subgrupo'
      FocusControl = DBEdit6
    end
    object Label12: TLabel
      Left = 422
      Top = 93
      Width = 51
      Height = 13
      Caption = 'Varia'#231#227'o'
      FocusControl = DBEdit7
    end
    object Label13: TLabel
      Left = 423
      Top = 55
      Width = 36
      Height = 13
      Caption = 'Marca'
      FocusControl = DBEdit8
    end
    object Label16: TLabel
      Left = 654
      Top = 17
      Width = 116
      Height = 13
      Caption = 'Saldo Estoque Atual'
      FocusControl = DBEdit9
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clRed
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object DBEdit1: TDBEdit
      Left = 8
      Top = 30
      Width = 114
      Height = 21
      TabStop = False
      Color = 16249066
      Ctl3D = True
      DataField = 'PRODICOD'
      DataSource = DataSource1
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentCtl3D = False
      ParentFont = False
      TabOrder = 0
    end
    object DBEdit2: TDBEdit
      Left = 127
      Top = 31
      Width = 120
      Height = 21
      TabStop = False
      Color = 16249066
      Ctl3D = True
      DataField = 'PRODA15CODANT'
      DataSource = DataSource1
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentCtl3D = False
      ParentFont = False
      TabOrder = 1
    end
    object DBEdit3: TDBEdit
      Left = 251
      Top = 31
      Width = 358
      Height = 21
      TabStop = False
      Color = 16249066
      Ctl3D = True
      DataField = 'PRODA60CODBAR'
      DataSource = DataSource1
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentCtl3D = False
      ParentFont = False
      TabOrder = 2
    end
    object DBEdit4: TDBEdit
      Left = 8
      Top = 68
      Width = 406
      Height = 21
      TabStop = False
      Color = 16249066
      Ctl3D = True
      DataField = 'PRODA60DESCR'
      DataSource = DataSource1
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentCtl3D = False
      ParentFont = False
      TabOrder = 3
    end
    object DBEdit5: TDBEdit
      Left = 8
      Top = 107
      Width = 200
      Height = 21
      TabStop = False
      Color = 16249066
      Ctl3D = True
      DataField = 'GRUPA60DESCR'
      DataSource = DataSource1
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentCtl3D = False
      ParentFont = False
      TabOrder = 4
    end
    object DBEdit6: TDBEdit
      Left = 214
      Top = 107
      Width = 200
      Height = 21
      TabStop = False
      Color = 16249066
      Ctl3D = True
      DataField = 'SUBGA60DESCR'
      DataSource = DataSource1
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentCtl3D = False
      ParentFont = False
      TabOrder = 5
    end
    object DBEdit7: TDBEdit
      Left = 419
      Top = 107
      Width = 352
      Height = 21
      TabStop = False
      Color = 16249066
      Ctl3D = True
      DataField = 'VARIA60DESCR'
      DataSource = DataSource1
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentCtl3D = False
      ParentFont = False
      TabOrder = 6
    end
    object DBEdit8: TDBEdit
      Left = 420
      Top = 69
      Width = 350
      Height = 21
      TabStop = False
      Color = 16249066
      Ctl3D = True
      DataField = 'MARCA60DESCR'
      DataSource = DataSource1
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentCtl3D = False
      ParentFont = False
      TabOrder = 7
    end
    object DBEdit9: TDBEdit
      Left = 652
      Top = 31
      Width = 118
      Height = 21
      TabStop = False
      Color = 16249066
      Ctl3D = True
      DataField = 'PSLDN3QTDE'
      DataSource = dsSQLProdutoSaldo
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clRed
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentCtl3D = False
      ParentFont = False
      TabOrder = 8
    end
  end
  object Principais: TGroupBox
    Left = 0
    Top = 82
    Width = 1099
    Height = 57
    Align = alTop
    Caption = 'Dados Principais'
    Color = 16249066
    Font.Charset = DEFAULT_CHARSET
    Font.Color = 8404992
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    TabOrder = 1
    object Label9: TLabel
      Left = 275
      Top = 14
      Width = 49
      Height = 13
      Caption = 'Terminal'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = 8404992
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label8: TLabel
      Left = 6
      Top = 14
      Width = 49
      Height = 13
      Caption = 'Empresa'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = 8404992
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label7: TLabel
      Left = 559
      Top = 14
      Width = 80
      Height = 13
      Caption = 'Data Abertura'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = 8404992
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label14: TLabel
      Left = 672
      Top = 14
      Width = 101
      Height = 13
      Caption = 'Data Fechamento'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = 8404992
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label18: TLabel
      Left = 464
      Top = 14
      Width = 26
      Height = 13
      Caption = 'Tipo'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = 8404992
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object btAjusteInventarioFiscal: TRxSpeedButton
      Left = 933
      Top = 12
      Width = 117
      Height = 40
      Cursor = crHandPoint
      Caption = 'Ajustar Estoque Fiscal para SPED'
      Flat = True
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        0000}
      GrayedInactive = False
      Layout = blGlyphLeft
      Margin = 0
      MarkDropDown = False
      NumGlyphs = 2
      ParentFont = False
      Transparent = True
      WordWrap = True
      OnClick = btAjusteInventarioFiscalClick
    end
    object ComboEmpresa: TDBLookupComboBox
      Left = 6
      Top = 28
      Width = 267
      Height = 21
      Ctl3D = True
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      KeyField = 'EMPRICOD'
      ListField = 'EMPRA60NOMEFANT'
      ListSource = DSEmpresa
      ParentCtl3D = False
      ParentFont = False
      TabOrder = 0
    end
    object ComboTerminal: TDBLookupComboBox
      Left = 276
      Top = 28
      Width = 184
      Height = 21
      Ctl3D = True
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      KeyField = 'TERMICOD'
      ListField = 'TERMA60DESCR'
      ListSource = DSTblTerminal
      ParentCtl3D = False
      ParentFont = False
      TabOrder = 1
    end
    object Data: TDateEdit
      Left = 558
      Top = 28
      Width = 108
      Height = 21
      NumGlyphs = 2
      TabOrder = 3
      OnKeyPress = DataKeyPress
    end
    object DBDateEdit1: TDBDateEdit
      Left = 672
      Top = 28
      Width = 121
      Height = 21
      DataField = 'INVDFECHAMENTO'
      DataSource = DSInventario
      Enabled = False
      NumGlyphs = 2
      TabOrder = 4
    end
    object ComboTipoInventario: TComboBox
      Left = 463
      Top = 27
      Width = 88
      Height = 21
      ItemHeight = 13
      TabOrder = 2
      OnChange = ComboTipoInventarioChange
      Items.Strings = (
        'Fiscal'
        'Gerencial')
    end
    object EditValorEstoque: TCurrencyEdit
      Left = 821
      Top = 26
      Width = 107
      Height = 21
      AutoSize = False
      TabOrder = 5
    end
  end
  object Panel1: TPanel
    Left = 0
    Top = 139
    Width = 1099
    Height = 71
    Align = alTop
    BevelOuter = bvNone
    Color = 16249066
    TabOrder = 2
    object GroupBox3: TGroupBox
      Left = 410
      Top = 2
      Width = 370
      Height = 69
      Caption = 'Digita'#231#227'o de Valores'
      Color = 16249066
      Font.Charset = DEFAULT_CHARSET
      Font.Color = 8404992
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
      TabOrder = 0
      object Label1: TLabel
        Left = 13
        Top = 25
        Width = 45
        Height = 13
        Caption = '&Produto'
        FocusControl = EditPesquisa
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 8404992
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label6: TLabel
        Left = 210
        Top = 25
        Width = 118
        Height = 13
        Caption = '&Qtde M'#225'x = 1000 Un'
        FocusControl = EditContagem
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clRed
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object EditPesquisa: TEdit
        Left = 11
        Top = 39
        Width = 191
        Height = 21
        TabOrder = 0
        OnChange = EditPesquisaChange
        OnKeyDown = EditPesquisaKeyDown
        OnKeyPress = EditPesquisaKeyPress
      end
      object EditContagem: TEdit
        Left = 206
        Top = 39
        Width = 147
        Height = 21
        MaxLength = 8
        TabOrder = 1
        OnEnter = EditContagemEnter
        OnKeyDown = EditContagemKeyDown
        OnKeyPress = EditPesquisaKeyPress
      end
    end
    object PanelConfig: TPanel
      Left = 3
      Top = 1
      Width = 393
      Height = 70
      BevelOuter = bvLowered
      ParentColor = True
      TabOrder = 1
      object RadioLeitorTeclado: TRadioGroup
        Left = 290
        Top = 2
        Width = 86
        Height = 65
        Caption = 'Usando'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 8404992
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ItemIndex = 1
        Items.Strings = (
          'Leitor'
          'Teclado')
        ParentFont = False
        TabOrder = 2
      end
      object RadioGroupContagem: TRadioGroup
        Left = 209
        Top = 2
        Width = 79
        Height = 65
        Caption = 'Contagem'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 8404992
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ItemIndex = 0
        Items.Strings = (
          '1'#170
          '2'#170
          '3'#170)
        ParentFont = False
        TabOrder = 1
      end
      object RadioGroupPesquisa: TRadioGroup
        Left = 17
        Top = 2
        Width = 190
        Height = 65
        Caption = 'Argumento de Pesquisa'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 8404992
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ItemIndex = 0
        Items.Strings = (
          'C'#243'digo Interno'
          'C'#243'digo Antigo'
          'C'#243'digo Barras')
        ParentFont = False
        TabOrder = 0
        OnClick = EditPesquisaChange
      end
    end
  end
  object GroupBox2: TGroupBox
    Left = 0
    Top = 344
    Width = 1099
    Height = 296
    Align = alClient
    Caption = 
      '&Invent'#225'rio    F2 - Altera Quantidade do produto selecionado  | ' +
      ' F3 - Excluir o produto selecionado '
    Color = 16249066
    Font.Charset = DEFAULT_CHARSET
    Font.Color = 8404992
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    TabOrder = 3
    TabStop = True
    object DBGridLista: TDBGrid
      Left = 2
      Top = 15
      Width = 1095
      Height = 279
      Hint = 
        'Clique com o bot'#227'o direito do mouse para visualizar as fun'#231#245'es d' +
        'ispoin'#237'veis...'#13#10'Dica:'#13#10'Se voc'#234' clicar no t'#237'tulo de alguma coluna' +
        ' o sistema ordenar'#225' os registros '#13#10'conforme a coluna selecionada' +
        '...'
      Align = alClient
      BorderStyle = bsNone
      Color = 16249066
      DataSource = DSInventario
      FixedColor = 10053171
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      Options = [dgTitles, dgColumnResize, dgColLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
      ParentFont = False
      ParentShowHint = False
      PopupMenu = PopupGrid
      ShowHint = True
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWhite
      TitleFont.Height = -11
      TitleFont.Name = 'Tahoma'
      TitleFont.Style = [fsBold]
      OnKeyDown = DBGridListaKeyDown
      OnTitleClick = DBGridListaTitleClick
      Columns = <
        item
          Expanded = False
          FieldName = 'PRODICOD'
          Width = 53
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'ProdutoLookUp'
          Width = 395
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'INVDDATA'
          Width = 88
          Visible = True
        end
        item
          Color = clWhite
          Expanded = False
          FieldName = 'INVICONTAGEM1'
          Visible = True
        end
        item
          Color = clWhite
          Expanded = False
          FieldName = 'INVICONTAGEM2'
          Width = 73
          Visible = True
        end
        item
          Color = clWhite
          Expanded = False
          FieldName = 'INVICONTAGEM3'
          Width = 74
          Visible = True
        end>
    end
    object Memo1: TMemo
      Left = 190
      Top = 85
      Width = 289
      Height = 84
      Lines.Strings = (
        'Memo1')
      TabOrder = 1
      Visible = False
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 640
    Width = 1099
    Height = 32
    Align = alBottom
    Color = 16249066
    TabOrder = 4
    object Label15: TLabel
      Left = 428
      Top = 9
      Width = 110
      Height = 13
      Caption = 'Totais em Qtde....>'
      FocusControl = DBEdit7
      Font.Charset = DEFAULT_CHARSET
      Font.Color = 8404992
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label17: TLabel
      Left = 179
      Top = 8
      Width = 109
      Height = 13
      Caption = 'Custo Atual R$....>'
      FocusControl = DBEdit7
      Font.Charset = DEFAULT_CHARSET
      Font.Color = 8404992
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lbProdutoConsulta: TLabel
      Left = 12
      Top = 8
      Width = 81
      Height = 13
      AutoSize = False
      Color = clSilver
      Font.Charset = DEFAULT_CHARSET
      Font.Color = 8404992
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
    end
    object TotalPriContagem: TEvDBNumEdit
      Left = 544
      Top = 5
      Width = 72
      Height = 21
      TabStop = False
      AutoHideCalculator = False
      DataField = 'TOTAL1'
      DataSource = DSSQLTotalContagem
      Glyph.Data = {
        
        C000}
      TabOrder = 0
    end
    object TotalSecContagem: TEvDBNumEdit
      Left = 617
      Top = 5
      Width = 72
      Height = 21
      TabStop = False
      AutoHideCalculator = False
      DataField = 'TOTAL2'
      DataSource = DSSQLTotalContagem
      Glyph.Data = {
        
        C000}
      TabOrder = 1
    end
    object TotalTercContagem: TEvDBNumEdit
      Left = 690
      Top = 5
      Width = 72
      Height = 21
      TabStop = False
      AutoHideCalculator = False
      DataField = 'TOTAL3'
      DataSource = DSSQLTotalContagem
      Glyph.Data = {
        
        C000}
      TabOrder = 2
    end
    object EditTotalCusto1: TEvDBNumEdit
      Left = 294
      Top = 4
      Width = 104
      Height = 21
      TabStop = False
      AutoHideCalculator = False
      DataField = 'TOTALCUSTO1'
      DataSource = DSSQLTotalContagem
      Glyph.Data = {
        
        C000}
      TabOrder = 3
    end
  end
  object ScrollBoxTopo: TScrollBox
    Left = 0
    Top = 0
    Width = 1099
    Height = 82
    Align = alTop
    BevelInner = bvNone
    BevelOuter = bvNone
    BorderStyle = bsNone
    Color = clBlack
    ParentColor = False
    TabOrder = 6
    object PanelCabecalho: TPanel
      Left = 0
      Top = 0
      Width = 1099
      Height = 82
      Align = alTop
      BevelOuter = bvNone
      Color = clBlack
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      object LabelTitulo: TLabel
        Left = 2
        Top = 3
        Width = 62
        Height = 26
        Caption = 'T'#237'tulo'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWhite
        Font.Height = -24
        Font.Name = 'Times New Roman'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Msg: TLabel
        Left = 0
        Top = 25
        Width = 1099
        Height = 16
        Align = alBottom
        Caption = 'Mensagem'
        Font.Charset = ANSI_CHARSET
        Font.Color = clYellow
        Font.Height = -13
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        Transparent = False
        Visible = False
      end
      object PanelNavigator: TPanel
        Left = 0
        Top = 50
        Width = 1099
        Height = 32
        Align = alBottom
        BevelOuter = bvNone
        Color = clSilver
        TabOrder = 0
        object AdvPanelNavigator: TAdvOfficeStatusBar
          Left = 0
          Top = 0
          Width = 1099
          Height = 32
          AnchorHint = False
          Align = alClient
          Panels = <
            item
              AppearanceStyle = psLight
              DateFormat = 'mm/dd/yyyy'
              Progress.BackGround = clNone
              Progress.Indication = piPercentage
              Progress.Min = 0
              Progress.Max = 100
              Progress.Position = 0
              Progress.Level0Color = clLime
              Progress.Level0ColorTo = 14811105
              Progress.Level1Color = clYellow
              Progress.Level1ColorTo = 13303807
              Progress.Level2Color = 5483007
              Progress.Level2ColorTo = 11064319
              Progress.Level3Color = clRed
              Progress.Level3ColorTo = 13290239
              Progress.Level1Perc = 70
              Progress.Level2Perc = 90
              Progress.BorderColor = clBlack
              Progress.ShowBorder = False
              Progress.Stacked = False
              TimeFormat = 'hh:mm:ss'
              Width = 50
            end>
          SimplePanel = False
          SizeGrip = False
          URLColor = clBlue
          Styler = AdvOfficeStatusBarOfficeStyler1
          Version = '1.3.0.2'
          object BtnFecharTela: TSpeedButton
            Tag = 3
            Left = 706
            Top = 4
            Width = 76
            Height = 25
            Cursor = crHandPoint
            Hint = 'Fechar Tela'
            Caption = '&Fechar'
            Flat = True
            Font.Charset = ANSI_CHARSET
            Font.Color = clBlack
            Font.Height = -11
            Font.Name = 'Tahoma'
            Font.Style = []
            Glyph.Data = {
              
              0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C}
            ParentFont = False
            OnClick = BtnFecharTelaClick
          end
          object SpeedButton1: TSpeedButton
            Tag = 3
            Left = 3
            Top = 3
            Width = 113
            Height = 25
            Cursor = crHandPoint
            Action = actExibir
            Flat = True
            Font.Charset = ANSI_CHARSET
            Font.Color = clBlack
            Font.Height = -11
            Font.Name = 'Tahoma'
            Font.Style = []
            Glyph.Data = {
              
              DD00DDDDDDDDDDDDDD00}
            Margin = 0
            ParentFont = False
            ParentShowHint = False
            ShowHint = True
            Spacing = 1
          end
          object LabelDiversos: TRxSpeedButton
            Left = 203
            Top = 3
            Width = 73
            Height = 25
            Cursor = crHandPoint
            DropDownMenu = PopupMenuExcluir
            Caption = 'Excluir'
            Flat = True
            Font.Charset = ANSI_CHARSET
            Font.Color = clBlack
            Font.Height = -11
            Font.Name = 'Tahoma'
            Font.Style = []
            Glyph.Data = {
              
              FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
            GrayedInactive = False
            Layout = blGlyphLeft
            Margin = 0
            MarkDropDown = False
            ParentFont = False
            Transparent = True
            WordWrap = True
          end
          object BTAtualiza: TRxSpeedButton
            Left = 324
            Top = 3
            Width = 112
            Height = 25
            Cursor = crHandPoint
            Action = actAtualizar
            DropDownMenu = PopupMenuAtualizaEstoque
            Caption = 'Atualizar Saldo'
            Flat = True
            Font.Charset = ANSI_CHARSET
            Font.Color = clBlack
            Font.Height = -11
            Font.Name = 'Tahoma'
            Font.Style = [fsBold]
            Glyph.Data = {
              
              0000}
            GrayedInactive = False
            Layout = blGlyphLeft
            Margin = 0
            MarkDropDown = False
            NumGlyphs = 2
            ParentFont = False
            Transparent = True
            WordWrap = True
          end
          object RxSpeedButton1: TRxSpeedButton
            Left = 453
            Top = 3
            Width = 81
            Height = 25
            Cursor = crHandPoint
            DropDownMenu = PopupMenuDiverso
            Caption = '&Diversos'
            Flat = True
            Font.Charset = ANSI_CHARSET
            Font.Color = clBlack
            Font.Height = -11
            Font.Name = 'Tahoma'
            Font.Style = []
            Glyph.Data = {
              
              C0C0C0C0C0C0C0C00000}
            GrayedInactive = False
            Layout = blGlyphLeft
            Margin = 0
            MarkDropDown = False
            ParentFont = False
            Transparent = True
            WordWrap = True
          end
          object LabelGravar: TSpeedButton
            Tag = 3
            Left = 123
            Top = 3
            Width = 73
            Height = 25
            Cursor = crHandPoint
            Action = actGravar
            Caption = '&Gravar'
            Flat = True
            Font.Charset = ANSI_CHARSET
            Font.Color = clBlack
            Font.Height = -11
            Font.Name = 'Tahoma'
            Font.Style = []
            Glyph.Data = {
              
              FF7FFF7FFF7F}
            Margin = 0
            ParentFont = False
            ParentShowHint = False
            ShowHint = True
            Spacing = 1
          end
        end
      end
      object ProgressBar: TProgressBar
        Left = 0
        Top = 41
        Width = 1099
        Height = 9
        Align = alBottom
        TabOrder = 1
      end
    end
  end
  object SQLProduto: TRxQuery
    DatabaseName = 'DB'
    SQL.Strings = (
      'SELECT'
      '  PRO.PRODICOD,'
      '  PRO.PRODA60CODBAR,'
      '  PRO.PRODA15CODANT,'
      '  PRO.PRODA60DESCR,'
      '  PRO.GRUPICOD,'
      '  PRO.SUBGICOD,'
      '  PRO.VARIICOD,'
      '  PRO.MARCICOD,'
      '  GRU.GRUPA60DESCR,'
      '  SUB.SUBGA60DESCR,'
      '  VAR.VARIA60DESCR,'
      '  MAR.MARCA60DESCR'
      'FROM'
      '  PRODUTO PRO'
      '  LEFT OUTER JOIN GRUPO GRU ON PRO.GRUPICOD = GRU.GRUPICOD'
      
        '  LEFT OUTER JOIN SUBGRUPO SUB ON PRO.GRUPICOD = SUB.GRUPICOD AN' +
        'D'
      '    PRO.SUBGICOD = SUB.SUBGICOD'
      
        '  LEFT OUTER JOIN VARIACAO VAR ON PRO.GRUPICOD = VAR.GRUPICOD AN' +
        'D'
      '    PRO.SUBGICOD = VAR.SUBGICOD AND'
      '    PRO.VARIICOD = VAR.VARIICOD'
      '  LEFT OUTER JOIN MARCA MAR ON PRO.MARCICOD = MAR.MARCICOD'
      'WHERE'
      '  %MPRODICOD AND'
      '  %MPRODA15CODANT AND'
      '  %MPRODA60CODBAR'
      '  ')
    Macros = <
      item
        DataType = ftString
        Name = 'MPRODICOD'
        ParamType = ptInput
        Value = '0=0'
      end
      item
        DataType = ftString
        Name = 'MPRODA15CODANT'
        ParamType = ptInput
        Value = '0=0'
      end
      item
        DataType = ftString
        Name = 'MPRODA60CODBAR'
        ParamType = ptInput
        Value = '0=0'
      end>
    Left = 403
    Top = 228
    object SQLProdutoPRODICOD: TIntegerField
      DisplayLabel = 'C'#243'digo'
      FieldName = 'PRODICOD'
      Origin = 'DB.PRODUTO.PRODICOD'
    end
    object SQLProdutoPRODA60CODBAR: TStringField
      DisplayLabel = 'Cod Barras'
      FieldName = 'PRODA60CODBAR'
      Origin = 'DB.PRODUTO.PRODA60CODBAR'
      FixedChar = True
      Size = 60
    end
    object SQLProdutoPRODA15CODANT: TStringField
      DisplayLabel = 'Cod Antigo'
      FieldName = 'PRODA15CODANT'
      Origin = 'DB.PRODUTO.PRODA15CODANT'
      FixedChar = True
      Size = 15
    end
    object SQLProdutoPRODA60DESCR: TStringField
      DisplayLabel = 'Descri'#231#227'o'
      FieldName = 'PRODA60DESCR'
      Origin = 'DB.PRODUTO.PRODA60DESCR'
      FixedChar = True
      Size = 60
    end
    object SQLProdutoGRUPICOD: TIntegerField
      FieldName = 'GRUPICOD'
      Origin = 'DB.PRODUTO.GRUPICOD'
    end
    object SQLProdutoSUBGICOD: TIntegerField
      FieldName = 'SUBGICOD'
      Origin = 'DB.PRODUTO.SUBGICOD'
    end
    object SQLProdutoVARIICOD: TIntegerField
      FieldName = 'VARIICOD'
      Origin = 'DB.PRODUTO.VARIICOD'
    end
    object SQLProdutoMARCICOD: TIntegerField
      FieldName = 'MARCICOD'
      Origin = 'DB.PRODUTO.MARCICOD'
    end
    object SQLProdutoGRUPA60DESCR: TStringField
      DisplayLabel = 'Grupo'
      FieldName = 'GRUPA60DESCR'
      FixedChar = True
      Size = 60
    end
    object SQLProdutoSUBGA60DESCR: TStringField
      DisplayLabel = 'Subgrupo'
      FieldName = 'SUBGA60DESCR'
      FixedChar = True
      Size = 60
    end
    object SQLProdutoVARIA60DESCR: TStringField
      DisplayLabel = 'Varia'#231#227'o'
      FieldName = 'VARIA60DESCR'
      FixedChar = True
      Size = 60
    end
    object SQLProdutoMARCA60DESCR: TStringField
      DisplayLabel = 'Marca'
      FieldName = 'MARCA60DESCR'
      FixedChar = True
      Size = 60
    end
  end
  object DataSource1: TDataSource
    AutoEdit = False
    DataSet = SQLProduto
    Left = 435
    Top = 228
  end
  object ActionList1: TActionList
    Left = 561
    Top = 6
    object actExibir: TAction
      Caption = '&Exibir Invent'#225'rio'
      OnExecute = actExibirExecute
      OnUpdate = actExibirUpdate
    end
    object actGravar: TAction
      Caption = '&Gravar no Invent'#225'rio'
      Checked = True
      OnExecute = actGravarExecute
      OnUpdate = actGravarUpdate
    end
    object actExcluirSelecionado: TAction
      Caption = 'Invent'#225'rio Selecionado'
      OnExecute = actExcluirSelecionadoExecute
      OnUpdate = actExcluirSelecionadoUpdate
    end
    object actAtualizar: TAction
      Caption = 'Atualizar Saldo Estoque pelo Invent'#225'rio'
      OnUpdate = actAtualizarUpdate
    end
    object ActGravar1: TDataSetPost
      Category = 'Dataset'
      Caption = '&Gravar'
      Hint = 'Gravar'
      ImageIndex = 7
      DataSource = DSSQLConsultaInventario
    end
    object ActAumentarSaldo: TAction
      Caption = 'Aumentar no Saldo de Estoque'
    end
  end
  object SQLTerminal: TRxQuery
    DatabaseName = 'DB'
    DataSource = DSEmpresa
    SQL.Strings = (
      'select * from TERMINAL'
      'where EMPRICOD = :EMPRICOD'
      'order by TERMA60DESCR')
    Macros = <>
    Left = 701
    Top = 6
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'EMPRICOD'
        ParamType = ptUnknown
      end>
    object SQLTerminalTERMICOD: TIntegerField
      FieldName = 'TERMICOD'
      Origin = 'DB.TERMINAL.TERMICOD'
    end
    object SQLTerminalEMPRICOD: TIntegerField
      FieldName = 'EMPRICOD'
      Origin = 'DB.TERMINAL.EMPRICOD'
    end
    object SQLTerminalTERMA60DESCR: TStringField
      FieldName = 'TERMA60DESCR'
      Origin = 'DB.TERMINAL.TERMA60DESCR'
      FixedChar = True
      Size = 60
    end
  end
  object DSTblTerminal: TDataSource
    DataSet = SQLTerminal
    Left = 729
    Top = 6
  end
  object SQLEmpresa: TRxQuery
    DatabaseName = 'DB'
    SQL.Strings = (
      'select * from EMPRESA'
      'where (%MEmpresa)')
    Macros = <
      item
        DataType = ftString
        Name = 'MEmpresa'
        ParamType = ptInput
        Value = '0=0'
      end>
    Left = 645
    Top = 6
    object SQLEmpresaEMPRICOD: TIntegerField
      FieldName = 'EMPRICOD'
      Origin = 'DB.EMPRESA.EMPRICOD'
    end
    object SQLEmpresaEMPRA60RAZAOSOC: TStringField
      FieldName = 'EMPRA60RAZAOSOC'
      Origin = 'DB.EMPRESA.EMPRA60RAZAOSOC'
      FixedChar = True
      Size = 60
    end
    object SQLEmpresaEMPRA60NOMEFANT: TStringField
      FieldName = 'EMPRA60NOMEFANT'
      Origin = 'DB.EMPRESA.EMPRA60NOMEFANT'
      FixedChar = True
      Size = 60
    end
  end
  object DSEmpresa: TDataSource
    DataSet = SQLEmpresa
    Left = 673
    Top = 6
  end
  object SQLGrupo: TRxQuery
    DatabaseName = 'DB'
    SQL.Strings = (
      'SELECT *'
      'FROM GRUPO')
    Macros = <>
    Left = 99
    Top = 308
    object SQLGrupoGRUPICOD: TIntegerField
      FieldName = 'GRUPICOD'
      Origin = 'DB.GRUPO.GRUPICOD'
    end
    object SQLGrupoGRUPA60DESCR: TStringField
      FieldName = 'GRUPA60DESCR'
      Origin = 'DB.GRUPO.GRUPA60DESCR'
      FixedChar = True
      Size = 60
    end
  end
  object SQLSubgrupo: TRxQuery
    DatabaseName = 'DB'
    SQL.Strings = (
      'SELECT *'
      'FROM SUBGRUPO')
    Macros = <>
    Left = 127
    Top = 308
    object SQLSubgrupoGRUPICOD: TIntegerField
      FieldName = 'GRUPICOD'
      Origin = 'DB.SUBGRUPO.GRUPICOD'
    end
    object SQLSubgrupoSUBGICOD: TIntegerField
      FieldName = 'SUBGICOD'
      Origin = 'DB.SUBGRUPO.SUBGICOD'
    end
    object SQLSubgrupoSUBGA60DESCR: TStringField
      FieldName = 'SUBGA60DESCR'
      Origin = 'DB.SUBGRUPO.SUBGA60DESCR'
      FixedChar = True
      Size = 60
    end
  end
  object SQLVariacao: TRxQuery
    DatabaseName = 'DB'
    SQL.Strings = (
      'SELECT *'
      'FROM VARIACAO')
    Macros = <>
    Left = 155
    Top = 308
    object SQLVariacaoGRUPICOD: TIntegerField
      FieldName = 'GRUPICOD'
      Origin = 'DB.VARIACAO.GRUPICOD'
    end
    object SQLVariacaoSUBGICOD: TIntegerField
      FieldName = 'SUBGICOD'
      Origin = 'DB.VARIACAO.SUBGICOD'
    end
    object SQLVariacaoVARIICOD: TIntegerField
      FieldName = 'VARIICOD'
      Origin = 'DB.VARIACAO.VARIICOD'
    end
    object SQLVariacaoVARIA60DESCR: TStringField
      FieldName = 'VARIA60DESCR'
      Origin = 'DB.VARIACAO.VARIA60DESCR'
      FixedChar = True
      Size = 60
    end
  end
  object SQLMarca: TRxQuery
    DatabaseName = 'DB'
    SQL.Strings = (
      'SELECT *'
      'FROM MARCA')
    Macros = <>
    Left = 183
    Top = 308
    object SQLMarcaMARCICOD: TIntegerField
      FieldName = 'MARCICOD'
      Origin = 'DB.MARCA.MARCICOD'
    end
    object SQLMarcaMARCA60DESCR: TStringField
      FieldName = 'MARCA60DESCR'
      Origin = 'DB.MARCA.MARCA60DESCR'
      FixedChar = True
      Size = 60
    end
  end
  object SQLInventario: TRxQuery
    BeforeOpen = SQLInventarioBeforeOpen
    AfterOpen = SQLInventarioAfterOpen
    OnCalcFields = SQLInventarioCalcFields
    DatabaseName = 'DB'
    SQL.Strings = (
      'SELECT *'
      'FROM'
      '  INVENTARIOESTOQUE INV'
      'WHERE (%MFILTRO) and (%MProduto)'
      '%Ordem'
      ''
      '')
    Macros = <
      item
        DataType = ftString
        Name = 'MFILTRO'
        ParamType = ptInput
        Value = '0=0'
      end
      item
        DataType = ftString
        Name = 'MProduto'
        ParamType = ptInput
        Value = '0=0'
      end
      item
        DataType = ftString
        Name = 'Ordem'
        ParamType = ptInput
      end>
    Left = 248
    Top = 440
    object SQLInventarioINVDFECHAMENTO: TDateTimeField
      FieldName = 'INVDFECHAMENTO'
      Origin = 'DB.INVENTARIOESTOQUE.INVDFECHAMENTO'
    end
    object SQLInventarioEMPRICOD: TIntegerField
      FieldName = 'EMPRICOD'
      Origin = 'DB.INVENTARIOESTOQUE.EMPRICOD'
    end
    object SQLInventarioTERMICOD: TIntegerField
      FieldName = 'TERMICOD'
      Origin = 'DB.INVENTARIOESTOQUE.TERMICOD'
    end
    object SQLInventarioPRODICOD: TIntegerField
      DisplayLabel = 'Produto'
      FieldName = 'PRODICOD'
      Origin = 'DB.INVENTARIOESTOQUE.PRODICOD'
    end
    object SQLInventarioINVDDATA: TDateTimeField
      DisplayLabel = 'Data'
      FieldName = 'INVDDATA'
      Origin = 'DB.INVENTARIOESTOQUE.INVDDATA'
      DisplayFormat = 'dd/mm/yyyy'
    end
    object SQLInventarioINVIPRODPOS: TIntegerField
      FieldName = 'INVIPRODPOS'
      Origin = 'DB.INVENTARIOESTOQUE.INVIPRODPOS'
    end
    object SQLInventarioINVICONTAGEM1: TFloatField
      DisplayLabel = 'Contagem 1'
      FieldName = 'INVICONTAGEM1'
      Origin = 'DB.INVENTARIOESTOQUE.INVICONTAGEM1'
      DisplayFormat = '###0.000'
    end
    object SQLInventarioINVICONTAGEM2: TFloatField
      DisplayLabel = 'Contagem 2'
      FieldName = 'INVICONTAGEM2'
      Origin = 'DB.INVENTARIOESTOQUE.INVICONTAGEM2'
      DisplayFormat = '###0.000'
    end
    object SQLInventarioINVICONTAGEM3: TFloatField
      DisplayLabel = 'Contagem 3'
      FieldName = 'INVICONTAGEM3'
      Origin = 'DB.INVENTARIOESTOQUE.INVICONTAGEM3'
      DisplayFormat = '###0.000'
    end
    object SQLInventarioPENDENTE: TStringField
      FieldName = 'PENDENTE'
      Origin = 'DB.INVENTARIOESTOQUE.PENDENTE'
      FixedChar = True
      Size = 1
    end
    object SQLInventarioREGISTRO: TDateTimeField
      FieldName = 'REGISTRO'
      Origin = 'DB.INVENTARIOESTOQUE.REGISTRO'
    end
    object SQLInventarioProdutoLookUp: TStringField
      DisplayLabel = 'Descri'#231#227'o'
      FieldKind = fkCalculated
      FieldName = 'ProdutoLookUp'
      Visible = False
      Size = 60
      Calculated = True
    end
    object SQLInventarioGradeCodLookUp: TIntegerField
      FieldKind = fkCalculated
      FieldName = 'GradeCodLookUp'
      Visible = False
      Calculated = True
    end
    object SQLInventarioAgrupGradeLookUp: TIntegerField
      FieldKind = fkCalculated
      FieldName = 'AgrupGradeLookUp'
      Visible = False
      Calculated = True
    end
    object SQLInventarioReferenciaLookUp: TStringField
      DisplayWidth = 60
      FieldKind = fkCalculated
      FieldName = 'ReferenciaLookUp'
      Size = 60
      Calculated = True
    end
    object SQLInventarioTIPO: TStringField
      FieldName = 'TIPO'
      Origin = 'DB.INVENTARIOESTOQUE.TIPO'
      FixedChar = True
      Size = 1
    end
  end
  object DSInventario: TDataSource
    AutoEdit = False
    DataSet = SQLInventario
    Left = 278
    Top = 440
  end
  object SQLConsultaInventario: TRxQuery
    Tag = 1
    BeforeEdit = SQLConsultaInventarioBeforeEdit
    BeforePost = SQLConsultaInventarioBeforePost
    AfterPost = SQLConsultaInventarioAfterPost
    OnNewRecord = SQLConsultaInventarioNewRecord
    DatabaseName = 'DB'
    RequestLive = True
    SQL.Strings = (
      'SELECT * FROM INVENTARIOESTOQUE'
      'WHERE %MFILTRO')
    Macros = <
      item
        DataType = ftString
        Name = 'MFILTRO'
        ParamType = ptInput
        Value = '0=0'
      end>
    Left = 589
    Top = 6
    object SQLConsultaInventarioEMPRICOD: TIntegerField
      Tag = 1
      FieldName = 'EMPRICOD'
      Origin = 'DB.INVENTARIOESTOQUE.EMPRICOD'
    end
    object SQLConsultaInventarioTERMICOD: TIntegerField
      Tag = 1
      FieldName = 'TERMICOD'
      Origin = 'DB.INVENTARIOESTOQUE.TERMICOD'
    end
    object SQLConsultaInventarioINVDDATA: TDateTimeField
      Tag = 1
      FieldName = 'INVDDATA'
      Origin = 'DB.INVENTARIOESTOQUE.INVDDATA'
    end
    object SQLConsultaInventarioINVIPRODPOS: TIntegerField
      FieldName = 'INVIPRODPOS'
      Origin = 'DB.INVENTARIOESTOQUE.INVIPRODPOS'
    end
    object SQLConsultaInventarioPRODICOD: TIntegerField
      FieldName = 'PRODICOD'
      Origin = 'DB.INVENTARIOESTOQUE.PRODICOD'
    end
    object SQLConsultaInventarioINVICONTAGEM1: TFloatField
      FieldName = 'INVICONTAGEM1'
      Origin = 'DB.INVENTARIOESTOQUE.INVICONTAGEM1'
    end
    object SQLConsultaInventarioINVICONTAGEM2: TFloatField
      FieldName = 'INVICONTAGEM2'
      Origin = 'DB.INVENTARIOESTOQUE.INVICONTAGEM2'
      OnChange = SQLConsultaInventarioINVICONTAGEM2Change
    end
    object SQLConsultaInventarioINVICONTAGEM3: TFloatField
      FieldName = 'INVICONTAGEM3'
      Origin = 'DB.INVENTARIOESTOQUE.INVICONTAGEM3'
      OnChange = SQLConsultaInventarioINVICONTAGEM3Change
    end
    object SQLConsultaInventarioPENDENTE: TStringField
      FieldName = 'PENDENTE'
      Origin = 'DB.INVENTARIOESTOQUE.PENDENTE'
      FixedChar = True
      Size = 1
    end
    object SQLConsultaInventarioREGISTRO: TDateTimeField
      FieldName = 'REGISTRO'
      Origin = 'DB.INVENTARIOESTOQUE.REGISTRO'
    end
    object SQLConsultaInventarioINVDFECHAMENTO: TDateTimeField
      FieldName = 'INVDFECHAMENTO'
      Origin = 'DB.INVENTARIOESTOQUE.INVDFECHAMENTO'
    end
    object SQLConsultaInventarioTIPO: TStringField
      FieldName = 'TIPO'
      Origin = 'DB.INVENTARIOESTOQUE.TIPO'
      FixedChar = True
      Size = 1
    end
  end
  object PopupMenuExcluir: TPopupMenu
    TrackButton = tbLeftButton
    Left = 233
    Top = 73
    object Produtoselecionado1: TMenuItem
      Caption = 'Produto selecionado'
      OnClick = Produtoselecionado1Click
    end
    object InventrioSelecionado1: TMenuItem
      Action = actExcluirSelecionado
    end
    object odooArquivoInventrio1: TMenuItem
      Caption = 'Todo o Arquivo Invent'#225'rio'
      OnClick = odooArquivoInventrio1Click
    end
  end
  object SQLExcluirInventario: TRxQuery
    DatabaseName = 'DB'
    RequestLive = True
    SQL.Strings = (
      'SELECT * FROM INVENTARIOESTOQUE '
      'WHERE %MFILTRO')
    Macros = <
      item
        DataType = ftString
        Name = 'MFILTRO'
        ParamType = ptInput
        Value = '0=0'
      end>
    Left = 533
    Top = 6
    object SQLExcluirInventarioEMPRICOD: TIntegerField
      FieldName = 'EMPRICOD'
      Origin = 'DB.INVENTARIOESTOQUE.EMPRICOD'
    end
    object SQLExcluirInventarioTERMICOD: TIntegerField
      FieldName = 'TERMICOD'
      Origin = 'DB.INVENTARIOESTOQUE.TERMICOD'
    end
    object SQLExcluirInventarioPRODICOD: TIntegerField
      FieldName = 'PRODICOD'
      Origin = 'DB.INVENTARIOESTOQUE.PRODICOD'
    end
    object SQLExcluirInventarioINVDDATA: TDateTimeField
      FieldName = 'INVDDATA'
      Origin = 'DB.INVENTARIOESTOQUE.INVDDATA'
    end
    object SQLExcluirInventarioINVIPRODPOS: TIntegerField
      FieldName = 'INVIPRODPOS'
      Origin = 'DB.INVENTARIOESTOQUE.INVIPRODPOS'
    end
    object SQLExcluirInventarioINVICONTAGEM1: TFloatField
      FieldName = 'INVICONTAGEM1'
      Origin = 'DB.INVENTARIOESTOQUE.INVICONTAGEM1'
    end
    object SQLExcluirInventarioINVICONTAGEM2: TFloatField
      FieldName = 'INVICONTAGEM2'
      Origin = 'DB.INVENTARIOESTOQUE.INVICONTAGEM2'
    end
    object SQLExcluirInventarioINVICONTAGEM3: TFloatField
      FieldName = 'INVICONTAGEM3'
      Origin = 'DB.INVENTARIOESTOQUE.INVICONTAGEM3'
    end
    object SQLExcluirInventarioPENDENTE: TStringField
      FieldName = 'PENDENTE'
      Origin = 'DB.INVENTARIOESTOQUE.PENDENTE'
      FixedChar = True
      Size = 1
    end
    object SQLExcluirInventarioREGISTRO: TDateTimeField
      FieldName = 'REGISTRO'
      Origin = 'DB.INVENTARIOESTOQUE.REGISTRO'
    end
    object SQLExcluirInventarioINVDFECHAMENTO: TDateTimeField
      FieldName = 'INVDFECHAMENTO'
      Origin = 'DB.INVENTARIOESTOQUE.INVDFECHAMENTO'
    end
    object SQLExcluirInventarioTIPO: TStringField
      FieldName = 'TIPO'
      Origin = 'DB.INVENTARIOESTOQUE.TIPO'
      FixedChar = True
      Size = 1
    end
  end
  object SQLProdutoSaldo: TRxQuery
    DatabaseName = 'DB'
    RequestLive = True
    SQL.Strings = (
      'SELECT * FROM PRODUTOSALDO'
      'WHERE %MFILTRO')
    Macros = <
      item
        DataType = ftString
        Name = 'MFILTRO'
        ParamType = ptInput
        Value = '0=0'
      end>
    Left = 617
    Top = 6
    object SQLProdutoSaldoEMPRICOD: TIntegerField
      FieldName = 'EMPRICOD'
      Origin = 'DB.PRODUTOSALDO.EMPRICOD'
    end
    object SQLProdutoSaldoPRODICOD: TIntegerField
      FieldName = 'PRODICOD'
      Origin = 'DB.PRODUTOSALDO.PRODICOD'
    end
    object SQLProdutoSaldoPSLDN3QTDE: TFloatField
      FieldName = 'PSLDN3QTDE'
      Origin = 'DB.PRODUTOSALDO.PSLDN3QTDE'
    end
    object SQLProdutoSaldoPSLDN3QTDMIN: TFloatField
      FieldName = 'PSLDN3QTDMIN'
      Origin = 'DB.PRODUTOSALDO.PSLDN3QTDMIN'
    end
    object SQLProdutoSaldoPSLDN3QTDMAX: TFloatField
      FieldName = 'PSLDN3QTDMAX'
      Origin = 'DB.PRODUTOSALDO.PSLDN3QTDMAX'
    end
    object SQLProdutoSaldoPENDENTE: TStringField
      FieldName = 'PENDENTE'
      Origin = 'DB.PRODUTOSALDO.PENDENTE'
      FixedChar = True
      Size = 1
    end
    object SQLProdutoSaldoREGISTRO: TDateTimeField
      FieldName = 'REGISTRO'
      Origin = 'DB.PRODUTOSALDO.REGISTRO'
    end
  end
  object SQLUsuario: TRxQuery
    DatabaseName = 'DB'
    RequestLive = True
    SQL.Strings = (
      'SELECT * FROM USUARIO'
      'WHERE %MFILTRO')
    Macros = <
      item
        DataType = ftString
        Name = 'MFILTRO'
        ParamType = ptInput
        Value = '0=0'
      end>
    Left = 505
    Top = 6
    object SQLUsuarioUSUAICOD: TIntegerField
      FieldName = 'USUAICOD'
      Origin = 'DB.USUARIO.USUAICOD'
    end
    object SQLUsuarioUSUAA60LOGIN: TStringField
      FieldName = 'USUAA60LOGIN'
      Origin = 'DB.USUARIO.USUAA60LOGIN'
      FixedChar = True
      Size = 60
    end
    object SQLUsuarioUSUAA5SENHA: TStringField
      FieldName = 'USUAA5SENHA'
      Origin = 'DB.USUARIO.USUAA5SENHA'
      FixedChar = True
      Size = 60
    end
    object SQLUsuarioUSUACVERDADCOMPRCLI: TStringField
      FieldName = 'USUACVERDADCOMPRCLI'
      Origin = 'DB.USUARIO.USUACVERDADCOMPRCLI'
      FixedChar = True
      Size = 1
    end
    object SQLUsuarioUSUACDESBLOQCLI: TStringField
      FieldName = 'USUACDESBLOQCLI'
      Origin = 'DB.USUARIO.USUACDESBLOQCLI'
      FixedChar = True
      Size = 1
    end
    object SQLUsuarioUSUAN2PERCDESC: TFloatField
      FieldName = 'USUAN2PERCDESC'
      Origin = 'DB.USUARIO.USUAN2PERCDESC'
    end
    object SQLUsuarioUSUACVENDCLIBLOQ: TStringField
      FieldName = 'USUACVENDCLIBLOQ'
      Origin = 'DB.USUARIO.USUACVENDCLIBLOQ'
      FixedChar = True
      Size = 1
    end
    object SQLUsuarioUSUACCANCVENDA: TStringField
      FieldName = 'USUACCANCVENDA'
      Origin = 'DB.USUARIO.USUACCANCVENDA'
      FixedChar = True
      Size = 1
    end
    object SQLUsuarioPENDENTE: TStringField
      FieldName = 'PENDENTE'
      Origin = 'DB.USUARIO.PENDENTE'
      FixedChar = True
      Size = 1
    end
    object SQLUsuarioREGISTRO: TDateTimeField
      FieldName = 'REGISTRO'
      Origin = 'DB.USUARIO.REGISTRO'
    end
    object SQLUsuarioUSUACEXCLCLI: TStringField
      FieldName = 'USUACEXCLCLI'
      Origin = 'DB.USUARIO.USUACEXCLCLI'
      FixedChar = True
      Size = 1
    end
    object SQLUsuarioUSUCALTERASLDINVENTARIO: TStringField
      FieldName = 'USUCALTERASLDINVENTARIO'
      Origin = 'DB.USUARIO.USUCALTERASLDINVENTARIO'
      FixedChar = True
      Size = 1
    end
  end
  object PopupMenuDiverso: TPopupMenu
    Left = 504
    Top = 64
    object ImportarProdutosdoColetorOptimusPadrao1: TMenuItem
      Caption = 'Importar Produtos do Coletor Optimus Padrao'
      OnClick = ImportarProdutosdoColetorOptimusPadrao1Click
    end
    object MnInvImportarProdutosdoColetor: TMenuItem
      Caption = 'Importar Produtos do Coletor Optimus'
      Visible = False
      OnClick = MnInvImportarProdutosdoColetorClick
    end
    object ImportardoPalmOne1: TMenuItem
      Caption = 'Importar Produtos do Coletor Palm One'
      OnClick = ImportardoPalmOne1Click
    end
    object ImportarBematechDC20001: TMenuItem
      Caption = 'Importar Produtos do Coletor Bematech DC2000'
      OnClick = ImportarBematechDC20001Click
    end
    object N3: TMenuItem
      Caption = '-'
    end
    object ZerarSaldoEstoquedosprodutosdesseinventario1: TMenuItem
      Caption = 'Zerar Saldo Estoque dos produtos desse inventario'
      OnClick = ZerarSaldoEstoquedosprodutosdesseinventario1Click
    end
  end
  object DSSQLConsultaInventario: TDataSource
    DataSet = SQLConsultaInventario
    Left = 548
    Top = 34
  end
  object PopupGrid: TPopupMenu
    Left = 320
    Top = 440
    object MnAlterarPrimeiraContagem: TMenuItem
      Caption = 'Alterar &Primeira Contagem'
      OnClick = MnAlterarPrimeiraContagemClick
    end
    object MnAlterarSegundaContagem: TMenuItem
      Tag = 1
      Caption = 'Alterar &Segunda Contagem'
      OnClick = MnAlterarPrimeiraContagemClick
    end
    object MnAlterarTerceiraContagem: TMenuItem
      Tag = 2
      Caption = 'Alterar &Terceira Contagem'
      OnClick = MnAlterarPrimeiraContagemClick
    end
    object N1: TMenuItem
      Caption = '-'
    end
    object ExcluirItemSelecionado1: TMenuItem
      Caption = 'Excluir Item Selecionado'
      OnClick = Produtoselecionado1Click
    end
  end
  object SQLExec: TRxQuery
    DatabaseName = 'DB'
    Macros = <>
    Left = 380
    Top = 6
  end
  object SQL: TRxQuery
    DatabaseName = 'DB'
    SQL.Strings = (
      'SELECT *'
      'FROM'
      '  INVENTARIOESTOQUE INV'
      'WHERE %MFILTRO'
      '%Ordem'
      ''
      '')
    Macros = <
      item
        DataType = ftString
        Name = 'MFILTRO'
        ParamType = ptInput
        Value = '0=0'
      end
      item
        DataType = ftString
        Name = 'Ordem'
        ParamType = ptInput
      end>
    Left = 248
    Top = 475
    object DateTimeField1: TDateTimeField
      FieldName = 'INVDFECHAMENTO'
      Origin = 'DB.INVENTARIOESTOQUE.INVDFECHAMENTO'
    end
    object IntegerField1: TIntegerField
      FieldName = 'EMPRICOD'
      Origin = 'DB.INVENTARIOESTOQUE.EMPRICOD'
    end
    object IntegerField2: TIntegerField
      FieldName = 'TERMICOD'
      Origin = 'DB.INVENTARIOESTOQUE.TERMICOD'
    end
    object IntegerField3: TIntegerField
      DisplayLabel = 'Produto'
      FieldName = 'PRODICOD'
      Origin = 'DB.INVENTARIOESTOQUE.PRODICOD'
    end
    object DateTimeField2: TDateTimeField
      DisplayLabel = 'Data'
      FieldName = 'INVDDATA'
      Origin = 'DB.INVENTARIOESTOQUE.INVDDATA'
      DisplayFormat = 'dd/mm/yyyy'
    end
    object IntegerField4: TIntegerField
      FieldName = 'INVIPRODPOS'
      Origin = 'DB.INVENTARIOESTOQUE.INVIPRODPOS'
    end
    object BCDField1: TFloatField
      DisplayLabel = 'Contagem 1'
      FieldName = 'INVICONTAGEM1'
      Origin = 'DB.INVENTARIOESTOQUE.INVICONTAGEM1'
      DisplayFormat = '###0.00'
    end
    object BCDField2: TFloatField
      DisplayLabel = 'Contagem 2'
      FieldName = 'INVICONTAGEM2'
      Origin = 'DB.INVENTARIOESTOQUE.INVICONTAGEM2'
      DisplayFormat = '###0.00'
    end
    object BCDField3: TFloatField
      DisplayLabel = 'Contagem 3'
      FieldName = 'INVICONTAGEM3'
      Origin = 'DB.INVENTARIOESTOQUE.INVICONTAGEM3'
      DisplayFormat = '###0.00'
    end
    object StringField1: TStringField
      FieldName = 'PENDENTE'
      Origin = 'DB.INVENTARIOESTOQUE.PENDENTE'
      FixedChar = True
      Size = 1
    end
    object DateTimeField3: TDateTimeField
      FieldName = 'REGISTRO'
      Origin = 'DB.INVENTARIOESTOQUE.REGISTRO'
    end
    object StringField2: TStringField
      DisplayLabel = 'Descri'#231#227'o'
      FieldKind = fkCalculated
      FieldName = 'ProdutoLookUp'
      Visible = False
      Size = 60
      Calculated = True
    end
    object IntegerField5: TIntegerField
      FieldKind = fkCalculated
      FieldName = 'GradeCodLookUp'
      Visible = False
      Calculated = True
    end
    object IntegerField6: TIntegerField
      FieldKind = fkCalculated
      FieldName = 'AgrupGradeLookUp'
      Visible = False
      Calculated = True
    end
    object StringField3: TStringField
      DisplayWidth = 60
      FieldKind = fkCalculated
      FieldName = 'ReferenciaLookUp'
      Size = 60
      Calculated = True
    end
    object SQLTIPO: TStringField
      FieldName = 'TIPO'
      Origin = 'DB.INVENTARIOESTOQUE.TIPO'
      FixedChar = True
      Size = 1
    end
  end
  object SQLConfigInventario: TRxQuery
    DatabaseName = 'DB'
    SQL.Strings = (
      'Select * from ConfigInventario')
    Macros = <>
    Left = 560
    Top = 456
    object SQLConfigInventarioOPESICODENTRADA: TIntegerField
      FieldName = 'OPESICODENTRADA'
      Origin = 'DB.CONFIGINVENTARIO.OPESICODENTRADA'
    end
    object SQLConfigInventarioOPESICODSAIDA: TIntegerField
      FieldName = 'OPESICODSAIDA'
      Origin = 'DB.CONFIGINVENTARIO.OPESICODSAIDA'
    end
  end
  object SQLTotalContagem: TRxQuery
    DatabaseName = 'DB'
    SQL.Strings = (
      'SELECT'
      '  sum(INVENTARIOESTOQUE.INVICONTAGEM1) as Total1,'
      '  sum(INVENTARIOESTOQUE.INVICONTAGEM2) as Total2,'
      '  sum(INVENTARIOESTOQUE.INVICONTAGEM3) as Total3,'
      
        '  sum(INVENTARIOESTOQUE.INVICONTAGEM1*PRODUTO.PRODN3VLRCUSTO) as' +
        ' TotalCusto1'
      'FROM'
      
        '  INVENTARIOESTOQUE INVENTARIOESTOQUE inner join Produto on INVE' +
        'NTARIOESTOQUE.PRODICOD = Produto.PRODICOD'
      'WHERE %MFILTRO'
      '')
    Macros = <
      item
        DataType = ftString
        Name = 'MFILTRO'
        ParamType = ptInput
        Value = '0=0'
      end>
    Left = 345
    Top = 607
    object SQLTotalContagemTOTAL1: TFloatField
      FieldName = 'TOTAL1'
      Origin = 'DB.INVENTARIOESTOQUE.INVICONTAGEM1'
    end
    object SQLTotalContagemTOTAL2: TFloatField
      FieldName = 'TOTAL2'
      Origin = 'DB.INVENTARIOESTOQUE.INVICONTAGEM2'
    end
    object SQLTotalContagemTOTAL3: TFloatField
      FieldName = 'TOTAL3'
      Origin = 'DB.INVENTARIOESTOQUE.INVICONTAGEM3'
    end
    object SQLTotalContagemTOTALCUSTO1: TFloatField
      FieldName = 'TOTALCUSTO1'
    end
  end
  object DSSQLTotalContagem: TDataSource
    AutoEdit = False
    DataSet = SQLTotalContagem
    Left = 313
    Top = 607
  end
  object PopupMenuAtualizaEstoque: TPopupMenu
    TrackButton = tbLeftButton
    Left = 350
    Top = 73
    object MnAdicionar: TMenuItem
      Caption = 'Adicionar'
      OnClick = MnAdicionarClick
    end
    object MnDiminuir: TMenuItem
      Caption = 'Diminuir'
      OnClick = MnDiminuirClick
    end
    object N2: TMenuItem
      Caption = '-'
    end
    object IncorporaraoNegativo1: TMenuItem
      Caption = 'Adicionar + Negativos'
      OnClick = IncorporaraoNegativo1Click
    end
  end
  object dsSQLProdutoSaldo: TDataSource
    AutoEdit = False
    DataSet = SQLProdutoSaldo
    Left = 619
    Top = 36
  end
  object FormStorage1: TFormStorage
    Options = []
    StoredProps.Strings = (
      'RadioGroupPesquisa.ItemIndex'
      'RadioGroupContagem.ItemIndex'
      'RadioLeitorTeclado.ItemIndex')
    StoredValues = <>
    Left = 160
    Top = 160
  end
  object AdvOfficeStatusBarOfficeStyler1: TAdvOfficeStatusBarOfficeStyler
    BorderColor = 14986888
    PanelAppearanceLight.BorderColor = 14922381
    PanelAppearanceLight.BorderColorHot = clGray
    PanelAppearanceLight.BorderColorDown = 10240783
    PanelAppearanceLight.Color = 16440774
    PanelAppearanceLight.ColorTo = 14854530
    PanelAppearanceLight.ColorHot = 13958143
    PanelAppearanceLight.ColorHotTo = 6538487
    PanelAppearanceLight.ColorDown = 9232890
    PanelAppearanceLight.ColorDownTo = 1940207
    PanelAppearanceLight.ColorMirror = 14854530
    PanelAppearanceLight.ColorMirrorTo = 14854530
    PanelAppearanceLight.ColorMirrorHot = 6538487
    PanelAppearanceLight.ColorMirrorHotTo = 6538487
    PanelAppearanceLight.ColorMirrorDown = 1940207
    PanelAppearanceLight.ColorMirrorDownTo = 1940207
    PanelAppearanceLight.TextColor = clBlack
    PanelAppearanceLight.TextColorHot = clBlack
    PanelAppearanceLight.TextColorDown = clBlack
    PanelAppearanceLight.TextStyle = []
    PanelAppearanceDark.BorderColor = clNone
    PanelAppearanceDark.BorderColorHot = clGray
    PanelAppearanceDark.BorderColorDown = 10240783
    PanelAppearanceDark.Color = 14986888
    PanelAppearanceDark.ColorTo = 14986888
    PanelAppearanceDark.ColorHot = 13958143
    PanelAppearanceDark.ColorHotTo = 6538487
    PanelAppearanceDark.ColorDown = 9232890
    PanelAppearanceDark.ColorDownTo = 1940207
    PanelAppearanceDark.ColorMirror = 14986888
    PanelAppearanceDark.ColorMirrorTo = 14986888
    PanelAppearanceDark.ColorMirrorHot = 6538487
    PanelAppearanceDark.ColorMirrorHotTo = 6538487
    PanelAppearanceDark.ColorMirrorDown = 1940207
    PanelAppearanceDark.ColorMirrorDownTo = 1940207
    PanelAppearanceDark.TextColor = clWhite
    PanelAppearanceDark.TextColorHot = clWhite
    PanelAppearanceDark.TextColorDown = clWhite
    PanelAppearanceDark.TextStyle = []
    Left = 412
    Top = 5
  end
end
