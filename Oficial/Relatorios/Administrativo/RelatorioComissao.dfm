inherited FormRelatorioComissao: TFormRelatorioComissao
  ...
  inherited TblTemporaria: TTable
    ...
    TableName = 'RelComissaoVendedor.db'
  ...
  object SQLVendedorComissao: TRxQuery
    ...
    SQL.Strings = (
      'select'
      '    VENDEDORCOMISSAO.VENDICOD,'
      '    sum(VENDEDORCOMISSAO.VDCON2TOTVENDVISTA) as VENDAVISTA,'
      '    sum(VENDEDORCOMISSAO.VDCON2TOTVENDPRAZO) as VENDAPRAZO,'
      
        '    sum(VENDEDORCOMISSAO.VDCON2TOTVENDVISTA) + sum(VENDEDORCOMIS' +
        'SAO.VDCON2TOTVENDPRAZO) as TOTALVENDA,'
      '    count(VENDEDORCOMISSAO.VDCOINROVEND) as NROVENDAS,'
      '    sum(VENDEDORCOMISSAO.VDCON3QTDVEND) as NTOITENS,'
      '    sum(VENDEDORCOMISSAO.VDCON2VLRCOMISS) as VALORCOMISSAO,'
      '    VENDEDOR.VENDA60NOME'
      'from'
      '   VENDEDORCOMISSAO, VENDEDOR'
      'where'
      '   (%MData) and'
      '   VENDEDORCOMISSAO.VENDICOD = VENDEDOR.VENDICOD'
      'group by'
      '    VENDEDORCOMISSAO.VENDICOD,'
      '    VENDEDOR.VENDA60NOME'
      ' ')
    ...
    object SQLVendedorComissaoNROVENDAS: TIntegerField
      FieldName = 'NROVENDAS'
      Origin = 'DB.VENDEDORCOMISSAO.VDCOINROVEND'
    end
