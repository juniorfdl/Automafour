...
begin
    ...
    Begin
      If (CorAnt<>SQLProdutoCORICOD.asString) or (GradeAnt<>SQLProdutoPRODIAGRUPGRADE.asString) Then
        Begin
          ...
          // Lanca o Saldo de Estoque caso o CkMostraSaldo esteja selecionado, para o empresario poder analisar melhor na recompra.
          if ckMostraSaldo.Checked then
            begin
              SQLSaldo.Close;
              SQLSaldo.MacrobyName('MEmpresa').Value := SQLDeLista(ComboEmpresa, ListaEmpresas, '', 'CUPOM', '');
              SQLSaldo.MacrobyName('MProduto').Value := 'PRODICOD = ' + SQLProdutoPRODICOD.AsString;
              SQLSaldo.Open;
              if not SQLSaldo.IsEmpty then
                begin
                  ...
                  TblTemporariaCORA60DESCR.asString    := SQLProdutoCORA30DESCR.asString + ' - ESTOQUE ATUAL =>';
  ...
  if ckMostraSaldo.Checked then
    Report.ReportName := DM.SQLConfigGeralCFGEA255PATHREPORT.Value + '\Vendas por Grade mais Saldo.rpt'
  else
