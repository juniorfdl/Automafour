...
Begin
  ...
  Else
    Begin
      ShowMessage('Você não adicionou Tabelas à consulta!');
      Exit;
    End;
...
begin
  ...
  if (SQLConsulta.FindField('CLIEA60EMAIL') <> nil) and (SQLConsulta.FindField('CLIEA60EMAIL').AsVariant <> null) then
    ...
    begin
      Application.MessageBox('O Campo CLIENTE.CLIEA60EMAIL não foi selecionado ou não possui dados, Impossível continuar !','Informação',MB_OK + MB_ICONINFORMATION + MB_SYSTEMMODAL);
      Abort;
    end;
end;
...
begin
  ...
  if Dm.SQLConfigCrediarioCFCRA254PATHMALADIRETA.AsString <> '' then
    begin
      if FileExists(Dm.SQLConfigCrediarioCFCRA254PATHMALADIRETA.AsString) then
        begin
          ppTextoMalaDireta.LoadFromFile(Dm.SQLConfigCrediarioCFCRA254PATHMALADIRETA.AsString);
