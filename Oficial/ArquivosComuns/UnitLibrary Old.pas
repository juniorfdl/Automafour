...
var
  ...
  ConsultandoPlano,
  AtualizarPedidos,
  EnviouNumerariosECF, ProcuraProdutoPelaRef08Char, Gravou, ImpValeTroco : boolean ;
  UltimoCodigo,
  NumerarioAtual,
  TerminalAtual,
  TerminalAnterior,
  ...
  NroViasTEF, vCLIEINDIACHQSJURO, CodMesa, CodConta, ProdutoAgrupGrade_MovDiv : Integer  ;
  CodiProduto,
  CodiCli,
  EstadoFechVendaAnt,
  CodigoProduto,
  ...
  OBSImpressaoCupom,
  MensagemVenda,
  TipoPlanoContas, TxtReciboECF, CodMotoboy,
  TipoHistoricoPadrao, ImprimeDadosClienteCupom, DataAniversario, Porta, TabelaMaisTerminal, PDV_OffLine, IDReimprimir: String;
  ...
  TotalDescGrade,
  VarValorTroco,
  VarValorRecebido, PerJuroPlanoAoMes, Vlr_Total_Pis, Vlr_Total_Cofins :  Double ;
  LogTotaliz        : TextFile ;
  ...
  NroUltItem : Integer;
  DataAtualizacaoEstoque : TDateTime;

function  IsOld(value : string) : boolean;
...
function  TestaNotaCompraMovimentoEstoque(IDNotaCompra, IDMovDiv, IDNotaVenda, Produto : String) : boolean;
...
procedure CalculaImpostosNotaFiscalItem(DataSet : TRXQuery; DataSource : TDataSource);
...
uses DataModulo, DataModuloTemplate, TelaImpressaoTermicaCodBar,
  WindowsLibrary, TelaAutenticaUsuario;
...
begin
  Result := False;
  if not FileExists('Preciosa.txt') then
    begin
      try
        if UpperCase(value[1]) = 'A' then
          begin
            StrToInt(value[2]);
            Result := True;
          end;
...
begin
 ...
 - DEVE-SE COLOCAR NAS POSIÇÕES ONDE DEVEM SER IMPRESSOS DADOS
  ...
  TblEtiquetas.DatabaseName    := 'UnitGestao_Temp';
...
begin
  ...
  if IDNotaCompra <> '' then
    MyQuery.SQL.Add('Select Count(*) as Contador from MovimentoEstoque Where NOCPA13ID = "'+IDNotaCompra+'" and PRODICOD = '+Produto) ;
  if IDMovDiv <> '' then
    MyQuery.SQL.Add('Select Count(*) as Contador from MovimentoEstoque Where MOVDA13ID = "'+IDMovDiv+'" and PRODICOD = '+Produto) ;
  if IDNotaVenda <> '' then
    MyQuery.SQL.Add('Select Count(*) as Contador from MovimentoEstoque Where NOFIA13ID = "'+IDNotaVenda+'" and PRODICOD = '+Produto) ;
  MyQuery.Open ;
...
begin
  ...
  DM.SQLTemplate.SQL.Add('And   NUMEICOD = ' + CodNumerario) ;
  DM.SQLTemplate.Open ;
  if DM.SQLTemplate.Fieldbyname('NUTCA15NUMERARIO').AsString <> '' then
    RetornaTotalizadorNumerarioECFDarumaFS345 := DM.SQLTemplate.Fieldbyname('NUTCA15NUMERARIO').AsString
  else
    begin
...
begin
  ...
  DM.SQLTemplate.Open ;
  if DM.SQLTemplate.Fieldbyname('NUTCA15NUMERARIO').AsString <> '' then
    RetornaTotalizadorNumerarioECFElgin_FIT := DM.SQLTemplate.Fieldbyname('NUTCA15NUMERARIO').AsString
  else
    begin
      Informa('Nenhum totalizador foi encontrado para o numerário ' + CodNumerario + '.');
...
begin
  DM.SQLTemplate.Close ;
  DM.SQLTemplate.SQL.Clear ;
  DM.SQLTemplate.SQL.Add('select NUMEA30DESCR from NUMERARIO') ;
  DM.SQLTemplate.SQL.Add('where NUMEICOD = ' + CodNumerario ) ;
  DM.SQLTemplate.Open ;
  if DM.SQLTemplate.Fieldbyname('NUMEA30DESCR').AsString <> '' then
    RetornaNumerario := DM.SQLTemplate.Fieldbyname('NUMEA30DESCR').AsString
  else
    RetornaNumerario := '';
end ;
    ...
    if not (Tabela as TQuery).IsEmpty then
      ...
      begin
        ...
        if not dm.SQLTemplate.IsEmpty then
          begin
            Codigo := dm.SQLTemplate.findfield('PRODICOD').AsString;
            CodigoBarrasProduto := dm.SQLTemplate.FindField('PRBAA15BARRAS').Value;
  ...
  TRY
   ...
   //RETORNA AUTOMAÇÃO
    ...
    begin
      SQLItemTroca.Close ;
      SQLItemTroca.SQL.Clear ;
      SQLItemTroca.SQL.Add('select c.cupodemis, b.converter, i.CPITN3QTD, i.CPITN3QTD*I.CPITN3VLRUNIT AS TOTAL, ') ;
      SQLItemTroca.SQL.Add('I.CPITN3VLRUNIT, I.ENCERRANTE FROM cupom c left JOIN CUPOMitem i on i.cupoa13id = c.cupoa13id ') ;
      SQLItemTroca.SQL.Add('left join bomba b on b.bombicod = i.bombicod where c.cupoa13id =  "' + Documento + '"' ) ;
      SQLItemTroca.Open ;

      while not SQLItemTroca.Eof do
      begin
        if not SQLItemTroca.FieldByName('Converter').IsNull then
        begin
          Dm.SQLTemplate.Close;
          Dm.SQLTemplate.SQL.Text := 'Select max(a.abastecida) as xabc from leiturapista a';
          Dm.SQLTemplate.Open;
          If DM.SQLTemplate.FieldByName('xabc').IsNull then
          begin
            DM.SQLTemplate.Close ;
            DM.SQLTemplate.SQL.Clear ;
            DM.SQLTemplate.SQL.Add('insert into LEITURAPISTA  (ABASTECIDA, DATA, CANAL, LITROS, VALORTOTAL, VALORUNITARIO, REGISTRO, MARCADO, ENCERRANTE) values ');
            Dm.SQLTemplate.Sql.Add('(:xAbastecida, :xDATA, :xCANAL, ');
            Dm.SQLTemplate.Sql.Add(':xLITROS, :xVALORTOTAL, :xVALORUNITARIO, :xREGISTRO, :xMARCADO, :xENCERRANTE)');
            DM.SQLTemplate.Prepare;
            Dm.SQLTemplate.ParamByName('xAbastecida').value := 100;
          end else
          begin
            DM.SQLTemplate.Close ;
            DM.SQLTemplate.SQL.Clear ;
            DM.SQLTemplate.SQL.Add('insert into LEITURAPISTA  (ABASTECIDA, DATA, CANAL, LITROS, VALORTOTAL, VALORUNITARIO, REGISTRO, MARCADO, ENCERRANTE) values ');
            Dm.SQLTemplate.Sql.Add('((Select max(a.abastecida)+1 from leiturapista a), :xDATA, :xCANAL, ');
            Dm.SQLTemplate.Sql.Add(':xLITROS, :xVALORTOTAL, :xVALORUNITARIO, :xREGISTRO, :xMARCADO, :xENCERRANTE)');
            DM.SQLTemplate.Prepare;
          end;
          Dm.SQLTemplate.ParamByName('xData').value         := SQLItemTroca.Fieldbyname('CUPODEMIS').Value;
          Dm.SQLTemplate.ParamByName('xCanal').value        := SQLItemTroca.Fieldbyname('Converter').Value;
          Dm.SQLTemplate.ParamByName('xlITROS').value       := SQLItemTroca.Fieldbyname('CPITN3QTD').Value;
          Dm.SQLTemplate.ParamByName('xVALORTOTAL').value   := SQLItemTroca.Fieldbyname('TOTAL').Value;
          Dm.SQLTemplate.ParamByName('xvalorunitario').value:= SQLItemTroca.Fieldbyname('CPITN3VLRUNIT').Value;
          Dm.SQLTemplate.ParamByName('xregistro').value     := 0;
          Dm.SQLTemplate.ParamByName('xMarcado').asstring   := 'N';
          if SQLItemTroca.Fieldbyname('ENCERRANTE').isnull then
            Dm.SQLTemplate.ParamByName('xencerrante').value   := 0
          else
            Dm.SQLTemplate.ParamByName('xencerrante').value   := SQLItemTroca.Fieldbyname('ENCERRANTE').VALUE;
          Dm.SQLTemplate.ExecSQL;
          Dm.SQLTemplate.Close;
        end;
        SQLItemTroca.Next;
...
begin
  ...
  If (Not PrecoOk) AND (TabelaPrecoCliente <> '') Then
    if (TabelaPrecoCliente <> 'V') and (TabelaPrecoCliente <> 'A') then
      Begin
...
begin
  ...
  try
    ...
    begin
      // IniFile.Create(DM.PathAplicacao + 'Terminal.ini');
      // Result := IniFile.ReadString('ComputerName','Name',Str);
      AssignFile(Texto,DM.PathAplicacao + 'Terminal.txt');
      Reset(Texto);
      Readln(Texto,Info);
      CloseFile(Texto);
      Result := Info;
    ...
    informa('Não foi possível encontrar o nome do computador! Por favor entre em contato com o suporte!');
  end;
      ...
      while not Gravou do
        begin
          ...
          try
            ...
            on E: Exception do
              begin
                ShowMessage('Entre em contato com o suporte da Conceitos Sistemas e comunique este erro: '+ #13 + E.Message);
                Inc(ProxCod) ;
              end;
          end ;
        end ;
...
begin
  if ((DataSet <> nil) and
    ...
    begin
      ...
      DataSet.FieldByName('NFITN2VLRLUCRO').AsFloat := (DataSet.FieldByName('NFITN2VLRUNIT').AsFloat - VALORCOMPRA) * DataSet.FieldByName('NFITN3QUANT').AsFloat;
      DataSet.FieldByName('NFITN2VLRDESC').AsFloat := (DataSet.FieldByName('NFITN2VLRUNIT').AsFloat * DataSet.FieldByName('NFITN3QUANT').AsFloat) * (DataSet.FieldByName('NFITN2PERCDESC').AsFloat / 100);
